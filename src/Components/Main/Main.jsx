...
import {
  ...
  setPageCount,
  setFilters,
} from '../../Redux/Slices/filter/filterSlice.js';
import qs from 'qs';
...
const Main = () => {
  ...
  const fetchPosts = () => {
    ...
    const checkerIncludes= sortSelectorId.property.split('').includes('-')
      ? 'asc'
      : 'desc';
  ...
  React.useEffect(() => {
    if (window.location.search) {
      ...
      const sort = Population.find(
        (el) => el.property === sortProperty,
      );
      dispatch(
        setFilters({
          SortId: sort,
  ...
  }, [vueSelectorId, sortSelectorId, pageSelectorCount, searchValue]);
  ...
  return (
    <div className="Main">
      <div className="MainContent">
        ...
        <div className="MainContentCatalog">{status === 'loading' ? renderSkelet : renderPizza}</div>
        <div className="MainContentPagination">
          <Pagination onChangePage={(el) => onChangePage(el)} />
        </div>
      </div>
    </div>
  );
