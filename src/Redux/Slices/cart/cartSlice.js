  ...
  reducers: {
    setOrders: (state, action) => {
      state.pizzas = state.pizzas.map((el) => {
        if (JSON.stringify(el) === JSON.stringify(action.payload.pizza)) {
          return { ...el, order: action.payload.currentCard.order };
        }
        if (JSON.stringify(el) === JSON.stringify(action.payload.currentCard)) {
          return { ...el, order: action.payload.pizza.order };
        }
        return el;
      });
      function sortFn(a, b) {
      ...
      state.pizzas = state.pizzas.sort(sortFn);
    },
    orderPizza: (state) => {
      state.pizzas = [];
